...
> 状态：🚧 进行中（M7.2 融合点积 ✅，M7.3 多线程 ✅，M7.4+ 待做）
...
| **M7.3 多线程分列** | 按输出列 j 分片，worker 私有 acc/rowBuf，`-threads` 配置（默认 8，12 核留余量） | `matmulQuantTransB` | ✅ decode 1.9→6.6、prefill 2.7→12.8 tokens/s |
...
- 线程数默认 **8**（12 核留 4 核给系统/编辑器），`-threads` flag 可覆盖；超过 CPU 核数自动裁剪。
